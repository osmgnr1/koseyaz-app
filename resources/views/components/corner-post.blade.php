<div class="my-2 p-2 rounded-md border-2 border-slate-400">

    <div class="mb-2">
        <h2 class="text-sm sm:text-lg font-medium font-sans text-slate-500 hover:text-slate-900"><a
                href="{{ route('cornerposts.show', ['cornerpost' => $cornerpost]) }}">{{ $cornerpost->title }}</a></h2>
    </div>

    <div class="mb-2 sm:mb-4 flex-col justify-between text-slate-500">
        ...
        <div class="flex gap-2 text-slate-500 items-center">
            ...
            <div class="pl-2">
                <p class="text-sm sm:text-base"><a
                        href="{{ route('user.profile', ['user' => $cornerpost->user]) }}">{{ $cornerpost->user->username }}</a>
                    tarafından</p>
            </div>
        </div>
        <div class="flex gap-2 text-slate-500 items-center">
            ...
            <div class="pl-2">
                <p class="text-sm sm:text-base">{{ \Carbon\Carbon::parse($cornerpost->published_at)->diffForHumans() }}
                    yayınlandı</p>
            </div>
        </div>
    </div>

    <div class="mb-2 sm:mb-4 text-slate-500 text-justify">
        <span class="text-sm sm:text-base"><a class="italic text-blue-600"
                href="{{ route('cornerposts.show', ['cornerpost' => $cornerpost]) }}">
                {{ __('See more...') }}</a>
        </span>
    </div>
        ...
        <div class="flex gap-2 items-center">
            <div>
                <span>
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1"
                        ...
                        enable-background="new 0 0 231.6208 232.1921" xml:space="preserve" width="30px" height="30px"
                        fill="#000000" stroke="#000000" stroke-width="0.002316208">
                        ...
                        <g id="SVGRepo_iconCarrier">
                            <g>
                                ...
                                <rect x="37.9222" y="88.0949"
                                    transform="matrix(0.7071 -0.7071 0.7071 0.7071 -37.5108 93.6329)" fill="#1C71DA"
                                    width="112.6869" height="7.9991"></rect>
                                <rect x="65.8187" y="123.4504"
                                    transform="matrix(0.7071 -0.7071 0.7071 0.7071 -59.6124 110.9902)" fill="#1C71DA"
                                    width="76.6907" height="7.9991"></rect>
                                <rect x="85.519" y="139.0051"
                                    transform="matrix(0.7071 -0.7071 0.7071 0.7071 -61.9119 136.5407)" fill="#1C71DA"
                                    width="96.6886" height="7.9991"></rect>
                            </g>
                            <path fill="#FF5D5D"
            ...
            <div class="grid grid-cols-2 sm:flex gap-0">
                @foreach ($cornerpost->tag as $tag)
                    <span
                        class="rounded-md border border-blue-300 px-1 mx-1 w-fit text-xs sm:text-sm text-slate-500 font-semibold shadow-sm italic">
                        {{ $tag->name }}</span>
                @endforeach
            </div>
        </div>
        <div>
            {{-- <button class="right-0 rounded-md border border-blue-300 px-2.5 py-1.5 text-center text-sm font-semibold shadow-sm hover:bg-blue-300"><a href="{{ route('categories.show', ['category' => $cornerpost->category]) }}">{{ strtoupper($cornerpost->category->name) }}</a></button> --}}
            <span
                class="right-0 rounded-md border border-blue-300 px-2.5 py-1.5 text-center text-xs sm:text-sm font-semibold shadow-sm">{{ strtoupper($cornerpost->category->name) }}</span>
        </div>
    </div>
    <div class="flex justify-end self-center space-x-3 py-2">
